...
def discover_entrypoint_manifests(_visit_budget=None) -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
    ...
    try:
        ...
        if _visit_budget is not None:
            groups = eps.values() if isinstance(eps, Mapping) else (eps,)
            for group in groups:
                for _entry_point in group:
                    _visit_budget.visit()
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if isinstance(raw_schema, str):
        # File-backed connector descriptors are parsed into ``configuration``
        # by load_plugin_configuration(); they are not the inline v2 schema.
        raw_schema = None
    elif raw_schema is not None and not isinstance(raw_schema, Mapping):
        logger.warning(
...
class PluginManager:
    ...
    def _unload_scoped(
        ...
        for key in target_keys:
            self._remove_plugin_subscriptions(key)
            self._application_command_providers.pop(key, None)
            self._setup_actions.pop(key, None)
        with self._background_service_lock:
            self._background_services = {
                name: registration
                for name, registration in self._background_services.items()
                if registration.plugin not in target_keys
            }
    ...
    def _scan_entry_points(
        ...
        *,
        ...
        """
        return discover_entrypoint_manifests(_visit_budget)
...
def get_plugin_manager() -> PluginManager:
    ...
    with _plugin_managers_lock:
        ...
        if (
            ...
            injected_home = Path(_plugin_manager.scope_key).expanduser().resolve()
            _plugin_managers_by_home[injected_home] = _plugin_manager
            if injected_home == current_home:
...
def resolve_pre_tool_admission(
    ...
    effective_args = details.modified_args or normalized_args
...
def _resolve_block_from_details(
    ...
    *,
    ...
    return PreToolAdmissionDecision(modified_args=details.modified_args)
...
def _dispatch_pre_tool_call_hooks(
    ...
    block_msg = _resolve_block_from_details(
        details,
        tool_name,
        normalized_args=details.modified_args or (args or {}),
        turn_id=turn_id, tool_call_id=tool_call_id, session_id=session_id,
    ).block_message
