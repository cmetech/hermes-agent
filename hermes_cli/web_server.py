...
async def put_plugin_providers(request: Request, body: _PluginProvidersPutBody):
    ...
    try:
        def _mutate_config() -> tuple[dict, dict]:
            with _CONFIG_MUTATION_LOCK:
                previous = copy.deepcopy(load_config())
                if body.memory_provider is not None:
                    memory_provider = _normalize_memory_provider_name(
                        body.memory_provider
                    )
                    _require_memory_provider_ready(memory_provider)
                    _save_memory_provider(memory_provider)
                if body.context_engine is not None:
                    _save_context_engine(body.context_engine)
                return previous, copy.deepcopy(load_config())

        previous_config, attempted_config = await asyncio.to_thread(_mutate_config)
        try:
            await _reload_plugin_background_services_or_raise(request)
        except HTTPException:
            try:
