...
import asyncio
import copy  # noqa: F401 — used by handlers
import logging
...
_conditional_provider_config_rollback = late("_conditional_provider_config_rollback")
_find_toolset_provider_row = late("_find_toolset_provider_row")
_probe_terminal_backend = late("_probe_terminal_backend")
_provider_reload_conflict = late("_provider_reload_conflict")
_reload_plugin_background_services = late("_reload_plugin_background_services")
...
_PROVIDER_RELOAD_LOCK = LateState("_PROVIDER_RELOAD_LOCK")
...
async def select_toolset_provider(
    ...
    def _run():
        with _CONFIG_MUTATION_LOCK:
            ...
            if body.capability is not None:
                cat = TOOL_CATEGORIES.get(name)
                providers = (
                    _visible_providers(cat, config, force_fresh=True) if cat else []
                )
                prov = next(
                    (p for p in providers if p.get("name") == body.provider), None
                )
                if prov is None:
                    raise HTTPException(
                        status_code=400,
        ...
        return previous_config, attempted_config, response

    def _rollback(previous_config, attempted_config):
        with _CONFIG_MUTATION_LOCK:
    ...
    with _config_profile_scope(body.profile or profile):
        ...
        try:
            previous_config, attempted_config, response = await asyncio.to_thread(_run)
            reload_result = await _reload_plugin_background_services(request.app)
            if not reload_result["ok"]:
                try:
                    await asyncio.to_thread(
                        _rollback, previous_config, attempted_config
                    )
