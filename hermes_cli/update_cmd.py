...
def _uv_subprocess_env() -> dict[str, str]:
    ...
    env = {**os.environ, "VIRTUAL_ENV": str(_m().PROJECT_ROOT / "venv")}
...
def _cmd_update_check(branch: str = "otto", *, branch_explicit: bool = False):
    ...
    ``branch`` selects which branch the check compares against. OTTO: the
    default is "otto" (mirrors ``_resolve_update_branch`` — a bare check must
    never compare against the upstream-mirror ``main``); callers can pass
    another branch to ask "are there new commits
