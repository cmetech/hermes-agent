    ...
    with write_txn(conn):
        ...
        if previous.status == "running" and new_status == "ready":
            resume_status = _retry_status_for_run(
                conn, task_id, previous.current_run_id
            )
        ...
        if previous.status == "running" and previous.current_run_id is not None:
            run_id = _end_run(
                ...
                outcome="reclaimed",
                status="reclaimed",
                summary=f"status changed to {effective_status} (operator)",
            )
            terminations.append((worker["worker_pid"], worker["claim_lock"]))
        _append_event(
            conn,
            task_id,
            "status",
...
def request_review(
    ...
    *,
    ...
    with write_txn(conn):
        if _check_mutation_precondition(conn, task_id, precondition) is None:
