...
if not _EARLY_READONLY_STARTUP:
    # Avoid mapping secret-manager native libraries into the updater process
    # before its child installer replaces the environment (#73381).
    load_hermes_dotenv(
...
def _read_packed_ref(common_dir: Path, ref: str) -> str | None:
    from hermes_cli.bytecode_cache import _read_packed_ref as read
    return read(common_dir, ref)


def _read_git_revision_fingerprint(repo_root: Path) -> str | None:
    from hermes_cli.bytecode_cache import _read_git_revision_fingerprint as read
...
def _clear_bytecode_cache(root: Path) -> int:
    # Shared with the install-only helper, which must clean before app imports.
    from hermes_cli.bytecode_cache import _clear_bytecode_cache as clear
    return clear(root)
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → {cli_name} -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  {cli_name} profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     {cli_name} -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `{cli_name} profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → {cli_name} -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  {cli_name} -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      {cli_name} -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    f"Only profiles installed via `{cli_name} profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"{cli_name} -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     {product_display_name()} {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: {product_display_name()} {mf.hermes_requires}")
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "gui", "desktop", "handoff", "kanban", "login", "logout", "logs", "lsp", "mcp", "memory", "migrate", "moa",
        ...
        "webhook", "whatsapp", "whatsapp-cloud", "worktree",
        "chat", "secrets", "security", "verify",
        "browser",
...
def _advertise_agent_env() -> None:
    """Advertise the agent harness to child processes without clobbering an outer one."""
...
def main() -> int:
    ...
    from hermes_cli.handoff.cli import build_handoff_parser

    build_handoff_parser(subparsers)
    ...
    # `hermes --version` / `-V` carries the full version and update status.
