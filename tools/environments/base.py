...
def _export_dump_excluding_session_vars(
    ...
    *,
    ...
    # ``hardened=True`` (the snapshot bootstrap / protected snapshot-update
    # contexts) routes every word through ``\builtin`` so login-profile or
    # snapshot-persisted functions named ``unset``/``export``/``true`` cannot
    # intercept the dump (see TestAtomicSnapshotWrite
    # test_bootstrap_bypasses_login_shell_operation_functions). The default
    # (non-hardened) output is byte-stable for direct callers/tests.
    unset_cmd = r"\builtin unset" if hardened else "unset"
    export_cmd = r"\builtin export" if hardened else "export"
    fallback_cmd = r"\builtin :" if hardened else "true"
    return (
        "{ ( "
        f"{unset_cmd} ${{!HERMES_SESSION_*}} ${{!HERMES_CRON_AUTO_DELIVER_*}} "
...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        _builtin = r"\builtin"
        # Per-session bridged vars (and profile-scoped passthrough names) must
        # never persist into the shared snapshot — see
        # ``_export_dump_excluding_session_vars`` (issues #71296 and the
        # cross-session HERMES_SESSION_ID leak).
        ...
        bootstrap = (
            ...
            f"{_export_dump_excluding_session_vars(_snap_tmp, snapshot_excluded, hardened=True)} &&\n"
    ...
    def _wrap_command(
        ...
        *,
        ...
        # Restore the current profile's passthrough values (or unset the
        # names) now that the shared snapshot — which may carry the PREVIOUS
        # profile's values — has been sourced. Must come after the source
        # group above; emits no output.
        ...
        # Preserve an outer harness identity while marking direct Hermes runs.
        parts.append(
            'builtin export AI_AGENT="${AI_AGENT:-hermes-agent}" '
        ...
        parts.append(
            'builtin export GIT_PAGER="${GIT_PAGER:-cat}" PAGER="${PAGER:-cat}"'
        )
        ...
        if self._snapshot_ready:
            snapshot_update.append(
                f"{{ __hermes_snap_tmp=$({_external} mktemp {_snap_tmp_template} 2>/dev/null) && "
                f"{_export_dump_excluding_session_vars(_snap_tmp, passthrough_names, hardened=True)} && "
        ...
        *,
        ...
        def _result(
            ...
            *,
            ...
            # Base dict (plus spill metadata when overflow occurred).
            result = self._finalize_wait_result(output, rendered, returncode)
            if sentinel_filter is not None:
        ...
        stdin_error = (
            str(stdin_errors[0])
            if isinstance(stdin_errors, list) and stdin_errors
            else ""
        )
        rendered = output.render(
            suffix=f"\n[stdin write failed: {stdin_error}]" if stdin_error else ""
        )
        result = _result(rendered, proc.returncode)
        if stdin_error:
            result["stdin_error"] = stdin_error
        return result
        ...
        *,
        ...
        if bounded.timed_out:
            ...
            result = {
                ...
                "returncode": 124,
                "_hermes_externally_stopped": True,
            }
        else:
