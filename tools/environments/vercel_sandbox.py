    ...
    def _run_bash(
        ...
        *,
        ...
        def exec_fn() -> tuple[str, int]:
            if clean:
                # Clean, profile-free shell with a scrubbed environment —
                # mirrors the Daytona/SSH remote backends (see base.py's
                # _CLEAN_OUTER_SHELL_PRELUDE for why the prelude runs first).
                wrapped = (
                ...
                args = ["-lc" if login else "-c", cmd_string]
            result = sandbox.run_command(
                "bash",
                args,
                cwd=workspace_root,
