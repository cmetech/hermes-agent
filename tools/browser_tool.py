...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    OTTO: an override naming a LOCAL enrolled browser is DROPPED here rather
    than honoured -- see ``_reject_enrolled_override``. It MUST be applied at
    this layer (not only in :func:`_get_cdp_override`): routing/availability
    sites such as ``_navigation_session_key`` and ``_is_local_backend`` consult
    this raw variant, and an enrolled-pointing override surviving here would
    hand every navigation the BARE key with the corporate endpoint (CRIT-001).
    The check itself performs no network I/O.
    """
...
def _get_cdp_override() -> str:
    ...
    OTTO: an override naming a LOCAL enrolled browser is DROPPED (by
    ``_get_cdp_override_raw`` via ``_reject_enrolled_override``) rather than
    honoured. Ordinary throwaway and remote CDP endpoints are unaffected, and
    env-over-config precedence is unchanged.
...
def _navigation_session_key(task_id: str, url: str) -> str:
    ...
    Checked in order:
      ...
      4. Otherwise, returns the bare task_id unless ALL of these are true:
         ...
         session (if any) continues to serve public URLs. With
         ``browser.use_real_profile`` consent, local sessions (bare or sidecar)
         attach to the user's real-profile copy-browser via
         ``_create_local_session``; forcing a local session from the model side
         is the Browser Use lane's ``local`` argument, not a built-in-tools
         argument.
    """
    if task_id is None:
        task_id = "default"
    # OTTO: deliberately the resolving variant, not _get_cdp_override_raw().
    # Truthiness is identical (resolution only normalizes a non-empty raw
    # value, and the enrolled-override rejection lives in the raw layer), no
    # network I/O happens when no override is configured, and the enrolled
    # routing tests pin this exact seam (test_browser_enrolled_routing.py
    # patches _get_cdp_override to simulate /browser connect).
