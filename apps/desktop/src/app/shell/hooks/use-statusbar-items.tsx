...
export function useStatusbarItems({
  ...
  const clientVersionItem = useMemo<StatusbarItem>(() => {
    ...
    // Release mode fakes `behind: 1` for legacy styling; zero it here and key
    // the hint off `updateAvailable` so the label reads `(update)`, not `(+1)`.
    const isRelease = updateStatus?.mode === 'release'

    const status = resolveVersionStatus({
      applying,
      applyMessage: updateApply.message,
      behind: isRelease ? 0 : (updateStatus?.behind ?? 0),
      ...
      target: 'client',
      updateAvailable: isRelease ? Boolean(updateStatus?.updateAvailable) : undefined,
