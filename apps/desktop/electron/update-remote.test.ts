 ...
 *
 * Run with: node --test electron/update-remote.ts
 * (Wired into npm test:desktop:platforms in package.json.)
 *
 * Why this matters: a public install can carry
 * origin=git@github.com:cmetech/hermes-agent.git. A background
 ...
 *
 * OTTO note: the "official" repo is the OTTO fork cmetech/hermes-agent, not
 * upstream nousresearch/hermes-agent (see OFFICIAL_REPO_CANONICAL flip in
 * update-remote.ts and the OTTO customization surface in CLAUDE.md).
 */
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@github.com:cmetech/hermes-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@github.com:cmetech/hermes-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@github.com/cmetech/hermes-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://github.com/cmetech/hermes-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@github.com:CMEtech/hermes-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://github.com/cmetech/hermes-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@github.com:cmetech/hermes-agent.git'), true)
  assert.equal(isSshRemote('ssh://git@github.com/cmetech/hermes-agent.git'), true)
  assert.equal(isSshRemote('https://github.com/cmetech/hermes-agent.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@github.com:cmetech/hermes-agent.git'), true)
  assert.equal(isOfficialSshRemote('git@github.com:cmetech/hermes-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@github.com/cmetech/hermes-agent.git'), true)
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  // Upstream (pre-fork) is not the OTTO official repo either.
  ...
  assert.equal(isOfficialSshRemote('git@gitlab.com:cmetech/hermes-agent.git'), false)
  ...
  assert.equal(isOfficialSshRemote('https://github.com/cmetech/hermes-agent.git'), false)
