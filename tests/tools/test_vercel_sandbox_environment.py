...
def _with_guard_attestation(script: str, result):
    """Reproduce the wrapper's guard-passed stdout attestation.

    The SDK fake stands in for Bash here: when the executed script carries a
    snapshot guard-passed sentinel, real bash would print it after the
    source/sanitizer/cwd setup succeed. Mirror that (like the Daytona test
    shim) so ``execute()`` does not demote a healthy fake session.
    """
    sentinel = re.search(r"__HERMES_SNAPSHOT_GUARD_PASSED_[0-9a-f]{32}__", script)
    if sentinel is None or not isinstance(result, _FakeRunResult):
        return result
    body = result._output
    if isinstance(body, bytes):
        body = body.decode("utf-8", errors="replace")
    return _FakeRunResult(f"\n{sentinel.group(0)}\n{body or ''}", result.exit_code)
    ...
    def run_command(self, cmd: str, args: list[str] | None = None, **kwargs):
        ...
        if self.run_command_side_effects:
            ...
            if isinstance(effect, Exception):
                raise effect
            if callable(effect):
                result = effect(cmd, args, kwargs)
            else:
                result = effect
        elif 'printf %s "$HOME"' in script:
            result = _FakeRunResult(self.home)
        else:
            result = _FakeRunResult("")
        return _with_guard_attestation(script, result)
