...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_defaults_to_otto_from_feature_branch(
        ...
        # the ff-merge of the fetched ref should use origin/otto (upstream
        # v0.20.0 replaced `git pull` with `merge --ff-only origin/<branch>`)
        merge_cmds = [c for c in commands if "merge --ff-only" in c]
        assert len(merge_cmds) == 1
        assert "origin/otto" in merge_cmds[0]
