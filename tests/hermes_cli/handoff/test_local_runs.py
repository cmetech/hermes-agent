...
@pytest.mark.asyncio
async def test_stop_stays_cancelling_until_a_poll_reports_terminal(profile_env):
    default_home, _target_home = profile_env
    async with _gateway(profile_env) as (_adapter, _server):
        service, created = _service(default_home)
        await asyncio.to_thread(service.advance, created.handoff_id)
        submitted = (
            await asyncio.to_thread(service.advance, created.handoff_id)
        ...
        for _ in range(30):
            ...
            if stopped.phase in {"cancelled", "succeeded", "failed"}:
                break
            await asyncio.sleep(0.02)
        assert stopped.phase in {"cancelled", "succeeded"}
...
@pytest.mark.asyncio
async def test_wildcard_listener_connects_through_ipv4_loopback(profile_env):
    default_home, _target_home = profile_env
    async with _gateway(profile_env) as (_adapter, server):
        _write_config(default_home, port=server.port, host="0.0.0.0")
        assessment = await asyncio.to_thread(
            AgentHandoffService(
