...
@pytest.mark.parametrize(
    ("basename", "expected"),
    [("hermes", "hermes"), (".otto", "otto"), ("loop24", "loop24")],
)
def test_product_cli_name_uses_source_stamped_brand(monkeypatch, basename, expected):
    monkeypatch.setattr(hermes_constants, "home_dir_basename", lambda: basename)

    assert hermes_constants.product_cli_name() == expected
