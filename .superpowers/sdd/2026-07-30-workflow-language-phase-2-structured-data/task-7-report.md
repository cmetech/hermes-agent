...
Task 7 is implemented in commit `908b00005`
(`refactor(workflow): centralize archon output resolution`). Archon downstream
...
Fresh result: 5 files, 63 passed, 0 failed.
...
```text
.venv/bin/ruff check plugins/workflow/output_resolution.py plugins/workflow/scheduler.py plugins/workflow/resources.py tests/plugins/workflow/test_scheduler.py tests/plugins/workflow/test_resources.py tests/plugins/workflow/test_language.py
git diff --check
```
