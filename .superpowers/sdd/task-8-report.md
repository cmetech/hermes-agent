...
- A separate scheduled continuation generation captures a fixed observed due
  instant and inclusive `created_at`/`run_id` high-water. Later admissions
  cannot extend that generation indefinitely.
...
- Every leadership term resets scheduled cursor, observed instant, and
  high-water state to page one. The durable periodic cursor remains independent.
