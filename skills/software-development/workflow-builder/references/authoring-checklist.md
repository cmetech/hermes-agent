...
Current/default Archon authoring and admission use normalizer v6; current
legacy uses v2. Explicit and sealed v1-v5 contracts remain compatible and keep
...
- Archon timeout and retry fields are supported: they were introduced in v3 and
  are inherited by current v6. Author
...
- MCP and skills remain node options. `include` is a compile-only v4 directive
  inherited by v6, not an executable node kind; ordinary new Archon admissions
  and generated contracts use v6.
- Compile and admit the generated v6 contract and preserve it in the sealed run
  snapshot. Use explicit v1-v5 only for compatibility with an intentionally
...
- For a v6 package, keep each `loop_group` body to one level,
