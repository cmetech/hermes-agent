...
param(
    ...
    [switch]$DiscardLocal,
    [string]$HermesHome = $(if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\loop24" }),
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\loop24\hermes-agent" }),
...
if ($PSBoundParameters.ContainsKey('HermesHome')) {
    ...
    $HermesHome = ConvertTo-LongPath $(
        if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\loop24" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:HERMES_HOME) { "$env:HERMES_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\loop24\hermes-agent" }
    )
}
...
#
# Hoisted rather than inlined because Install-CommandShims writes the SAME list
# into every launcher it generates. The install-time scrub and the runtime
# launcher must not be able to drift apart.
$InheritedPythonEnvVars = @('PYTHONHOME', 'PYTHONPATH', 'PYTHONSTARTUP', 'PYTHONEXECUTABLE', 'PYTHONUSERBASE')

$ScrubbedPythonEnvVars = @()
foreach ($pyVar in $InheritedPythonEnvVars) {
...
$RepoUrlSsh = "git@github.com:cmetech/hermes-agent.git"
$RepoUrlHttps = "https://github.com/cmetech/hermes-agent.git"
$PythonVersion = "3.11"
...
function Install-Repository {
    ...
    if (-not $didUpdate) {
        ...
        # Try SSH first, then HTTPS. Disable line-ending conversion before the
        # initial checkout because authenticated resources are hashed by byte.
...
# Command launchers
# ============================================================================
# The top-of-file scrub protects the INSTALL. It does not protect the command a
# user later types: pip's console scripts (venv\Scripts\hermes.exe and the
# branded siblings) start the venv interpreter directly, inheriting whatever
# Python environment the shell carries. A corporate PYTHONPATH naming a complete
# 3.10 stdlib is prepended AHEAD of the venv's own Lib, so `re` resolves to 3.10
# while the running binary is 3.11 and the process dies inside sre_compile.py
# with "AssertionError: SRE module mismatch". That happens in site.py during
# interpreter startup -- there is no in-process fix, because no hermes_cli code
# has run or can run.
#
# install.sh has always written such a launcher (setup_path). Windows had none;
# Set-PathVariable simply put venv\Scripts on PATH raw. This closes that gap.
#
# The launchers cannot live in venv\Scripts: within a single directory PATHEXT
# resolves .EXE before .CMD, so pip's exe would keep winning. They go in
# $HermesHome\bin, which Set-PathVariable places on PATH AHEAD of venv\Scripts
# -- across directories the first PATH entry holding any match wins, and that
# ordering is the whole mechanism.
#
# Known trade-off: a .cmd wrapper is a resident process, so Ctrl+C during an
# interactive session can raise cmd.exe's "Terminate batch job (Y/N)?" prompt.
# install.sh avoids this with exec, which cmd has no equivalent for. Correctness
# wins: without the launcher the command does not start at all.
function Get-ConsoleScriptNames {
    param([string]$PyprojectPath)

    # Read the names from [project.scripts] rather than hardcoding them. That
    # table is what the brand generator stamps, so a brand branch's own scripts
    # (otto, loop24-acp, ...) are picked up here with no per-brand edit.
    ...
    foreach ($line in (Get-Content -LiteralPath $PyprojectPath)) {
        $trimmed = $line.Trim()
        if ($trimmed -like '[[]*') {
            $inScripts = ($trimmed -eq '[project.scripts]')
            continue
        }
        if (-not $inScripts) { continue }
        if ($trimmed -match '^([A-Za-z0-9._-]+)\s*=') { $names += $Matches[1] }
    }
    return $names
}
...
function Install-CommandShims {
    ...
    foreach ($name in $Names) {
        # Only shim what was actually built. A declared-but-missing entry point
        # would otherwise get a launcher that shadows nothing and fails oddly.
        $target = Join-Path $ScriptsDir "$name.exe"
        if (-not (Test-Path $target)) { continue }

        $lines = @(
            '@echo off',
            'rem Generated by install.ps1 (Install-CommandShims). Clears the',
            'rem inherited Python environment before starting the venv entry',
            'rem point; an inherited PYTHONPATH/PYTHONHOME kills it in site.py.'
        )
        foreach ($pyVar in $InheritedPythonEnvVars) {
            $lines += "set `"$pyVar=`""
        }
        $lines += 'set "PYTHONNOUSERSITE=1"'
        # Quoted so a relocated HERMES_HOME containing a space still launches.
        $lines += "`"$target`" %*"
        $lines += 'exit /b %ERRORLEVEL%'

        Set-Content -LiteralPath (Join-Path $ShimDir "$name.cmd") -Value $lines -Encoding ASCII
        $written += $name
    }
    return $written
}
...
function Set-PathVariable {
    ...
    # Launchers that scrub the inherited Python environment, ahead of the raw
    # pip entry points. See Install-CommandShims for why the ordering matters.
    $shimDir = Join-Path $HermesHome "bin"
    $shims = Install-CommandShims -ShimDir $shimDir -ScriptsDir $hermesBin `
        -Names (Get-ConsoleScriptNames (Join-Path $InstallDir "pyproject.toml"))
    if ($shims.Count -gt 0) {
        Write-Success ("Installed command launchers: " + ($shims -join ", "))
    } else {
        Write-Warn "No command launchers written; commands will run pip's entry points directly"
    }

    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")

    # Remove the old whole-venv PATH entry so Hermes never shadows the user's
    # global python/pip commands (#83797).
    $legacyBin = "$InstallDir\venv\Scripts"
    if (-not $NoVenv) {
        $currentPath = (($currentPath -split ';' | Where-Object {
            $_ -and $_.TrimEnd('\') -ine $legacyBin.TrimEnd('\')
        }) -join ';')
    }

    # Add both dirs to user PATH, shim dir FIRST. Re-seat rather than skip when
    # they are already present so the environment-scrubbing shims always win.
    $ourDirs = @($shimDir, $hermesBin)
    $existing = @()
    if ($currentPath) {
        $existing = @($currentPath -split ';' | Where-Object {
            $entry = $_
            if (-not $entry) { return $false }
            -not ($ourDirs | Where-Object { $_.TrimEnd('\') -ieq $entry.TrimEnd('\') })
        })
    }
    $newPath = (($ourDirs + $existing) -join ';')

    if ($newPath -ne $currentPath) {
        [Environment]::SetEnvironmentVariable("Path", $newPath, "User")
        Write-Success "Added to user PATH: $shimDir (ahead of $hermesBin)"
    } else {
    ...
    $defaultHermesHome = "$env:LOCALAPPDATA\loop24"   # per-brand default (home emitter rewrites the segment)
    ...
    $env:Path = "$shimDir;$hermesBin;$env:Path"
