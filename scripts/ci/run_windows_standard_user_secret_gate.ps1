...
function Read-RedactedChildOutput([string]$path) {
    ...
    foreach ($line in $observed) {
        ...
        elseif ($line -match '^teams-cache-round-trip FAIL reason=teams-((first-persist|first-read|replacement-persist|replacement-read)-(outer|open-directory|close-directory|open-file|create-file|close-file|write-file|flush-file|publish-(file|disarm-delete|rename-file(-(access-denied|sharing-violation|invalid-parameter|not-found|reparse|other))?|verify-metadata|verify-user|verify-acl)|read-file|missing|mismatch)|first-acl-directory|first-acl-file|replacement-acl-file|cleanup-file|cleanup-directory)$') {
            $lines += $line
        }
