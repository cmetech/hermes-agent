...
# Configuration
REPO_URL_SSH="git@github.com:cmetech/hermes-agent.git"
REPO_URL_HTTPS="https://github.com/cmetech/hermes-agent.git"
HERMES_HOME="${HERMES_HOME:-$HOME/.loop24}"
...
# Print the console-script names declared in a pyproject.toml, one per line.
#
# Read from [project.scripts] rather than hardcoded, because that table is what
# the brand generator stamps: a brand branch adds `loop24`, `loop24-agent` and
# `loop24-acp` there, and they must become real commands with no per-brand edit
# here. install.ps1's Get-ConsoleScriptNames reads the same table for the same
# reason.
console_script_names() {
    local pyproject="$1"
    [ -f "$pyproject" ] || return 0
    awk '
        /^\[/ { in_scripts = ($0 ~ /^\[project\.scripts\]/); next }
        !in_scripts { next }
        /^[[:space:]]*#/ { next }
        /=/ {
            split($0, parts, "=")
...
# Write one launcher per console script into $link_dir.
#
# Previously only `hermes` was linked, so a branded install had no branded
# command: pip built venv/bin/loop24 and nothing on PATH ever pointed at it.
#
# Each launcher clears the inherited Python environment before exec'ing its own
# entry point. PYTHONPATH is prepended AHEAD of the interpreter's own Lib and
# PYTHONHOME replaces it outright, so either one pointed at a different minor
# version kills the process in site.py with "SRE module mismatch" before any
# hermes_cli code runs. The list mirrors the top-of-file scrub,
# INHERITED_PYTHON_ENV_VARS (apps/desktop/electron/backend-env.ts) and
# $InheritedPythonEnvVars (install.ps1's Install-CommandShims), which writes the
# Windows equivalent of these files.
#
# `exec` matters: the launcher replaces itself with the entry point rather than
# staying resident, so signals and exit codes pass through untouched. The
# Windows .cmd equivalent cannot do this, which is its one known wart.
write_command_launchers() {
    ...
    # Never shim a directory into itself. With --no-venv, $HERMES_BIN comes from
    # `which hermes`, which can already resolve inside $link_dir; writing a
    # launcher there that exec's "$bin_dir/$name" would exec itself forever.
    if [ "$(cd "$link_dir" 2>/dev/null && pwd -P)" = "$(cd "$bin_dir" 2>/dev/null && pwd -P)" ]; then
        return 0
    fi
    ...
    local written=""
    for name in "$@"; do
        # Only shim what pip actually built. A declared-but-absent entry point
        # would otherwise get a launcher that exec's a missing file.
        [ -x "$bin_dir/$name" ] || continue

        # Older installs created this path as a symlink to the venv entry point.
        # Without the rm, `cat >` follows the symlink and overwrites the pip
        # entry point with this shim -- making `exec` self-recurse. (#21454)
        rm -f "$link_dir/$name"
        if [ -n "$python_bin" ]; then
            # uv's POSIX console scripts use a shell trampoline that calls the
            # non-standard `realpath` utility. Stock macOS does not ship it.
            # The trampoline is also valid Python source, so invoking it with
            # the venv interpreter preserves every declared entry point without
            # depending on realpath (including branded commands).
            cat > "$link_dir/$name" <<EOF
#!/usr/bin/env bash
# hermes-agent command launcher (generated by scripts/install.sh)
unset PYTHONHOME
unset PYTHONPATH
unset PYTHONSTARTUP
...
setup_path() {
    ...
    # Verify the interpreter or console script needed by the launcher.
    ...
    # Create user-facing launchers for every console script the install built.
    # See write_command_launchers for why they scrub the environment, and
    # console_script_names for why the list comes from [project.scripts].
    local entry_point_dir
    entry_point_dir="$(dirname "$HERMES_BIN")"

    local script_names
    script_names="$(console_script_names "$INSTALL_DIR/pyproject.toml")"
    # Fall back to the one command we know exists if pyproject is unreadable,
    # so a parse problem degrades to the old behaviour instead of no command.
    [ -n "$script_names" ] || script_names="hermes"

    local installed
    local launcher_python=""
    [ "$USE_VENV" = true ] && launcher_python="$HERMES_BIN"
    installed="$(write_command_launchers "$command_link_dir" "$entry_point_dir" "$launcher_python" $script_names)"

    if [ -n "$installed" ]; then
        log_success "Installed launchers in $command_link_display_dir: $installed"
    else
        log_warn "No command launchers were installed in $command_link_display_dir"
    fi
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if npm_deps_current "$root_marker" "$root_fingerprint"; then
            ...
            cd "$INSTALL_DIR"
            # Scope the CLI install away from apps/desktop's native node-pty;
            # install_desktop owns that workspace.
