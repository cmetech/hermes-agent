...
# platform paths come from LOCALAPPDATA/APPDATA, machine-wide caches use
# ProgramData/SYSTEMDRIVE, ssl/sockets need SYSTEMROOT, and tempfile needs
# TEMP/TMP. Dropping them breaks collection on native Windows (issues #67385,
# #70813), and some native programs interpret an unexpanded ``%SystemDrive%``
# as a relative directory inside the checkout. These are location variables,
# not credentials, so forwarding them keeps the isolation intent intact. Each
# is only forwarded when actually set, so POSIX runs are byte-for-byte unchanged.
WIN_ENV=()
for _win_var in USERPROFILE HOMEDRIVE HOMEPATH LOCALAPPDATA APPDATA \
  ProgramData SYSTEMDRIVE SYSTEMROOT TEMP TMP; do
  if [ -n "${!_win_var:-}" ]; then
...
for _test_var in HERMES_TEST_IMAGE HERMES_TEST_WORKERS HERMES_TEST_PATHS \
  ...
  ERICSSON_CAPABILITIES_DIR ERICSSON_CAPABILITIES_EXPECTED_SHA; do
  if [ -n "${!_test_var:-}" ]; then
