...
# - plugin-agent / subagent — isolated workers and delegated child sessions are
#   hidden from every picker, so their
#   title is never read. A batch at `max_concurrent_children` would pay N title
#   calls for N names nobody sees.
_UNTITLED_PLATFORMS = frozenset({"cron", "plugin-agent", "subagent"})
