...
The ledger checker classifies incoming overlap as `none`, `same_file`, `owned_symbol`, or `possible_upstream_equivalent`. For every non-`none` entry it prints the rationale, owned contracts, merge guidance, removal condition, and exact invariant tests. The merge procedure preserves a pre-merge customization patch/evidence reference until post-merge tests pass, refuses to advance `last_verified_upstream` while conflicts/tests remain unresolved, and verifies that branded branches contain the tested `base` commit plus generated overlays rather than divergent copies of generic runtime code.
...
- Merge-rehearsal test from the latest upstream mirror through `base`, including ledger-owned non-overlap, textual overlap, owned-symbol conflict, and upstream-equivalent scenarios; then validate OTTO and LOOP24 ancestry, brand generation, and focused test suites.
