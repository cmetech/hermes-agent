...
def _discover_entry_point_providers() -> None:
    ...
    * **Provider targets only.** The ``hermes_agent.plugins`` group is shared
    ...
    enabled, disabled = _entry_point_plugin_gate()
    if not enabled:
        return
    ...
    for ep in group_eps:
        if ep.name not in enabled or ep.name in disabled:
            ...
            continue
        context = _RegistrationContext(
            origin_kind="entry_point",
            distribution_id=ep.name,
            distribution_version=str(getattr(getattr(ep, "dist", None), "version", "")),
            package_root=None,
        )
        token = _REGISTRATION_CONTEXT.set(context)
        try:
...
def _entry_point_plugin_gate() -> tuple[set[str] | None, set[str]]:
    """Read the entry-point allow/deny lists without loading plugin runtime."""
    try:
