...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def get_existing_session_conversation(
        ...
        """
        # Qualify every projected column with the messages alias: the shared
        # column list is written for single-table SELECTs, and the LEFT JOIN
        # against sessions makes bare ``id`` ambiguous.
        _qualified_columns = ", ".join(
            f"m.{column.strip()}"
            for column in self._CONVERSATION_ROW_COLUMNS.split(",")
        )
        with self._lock:
            rows = self._conn.execute(
                f"SELECT m.id AS _message_id, {_qualified_columns} "
                "FROM sessions AS s "
